...
static void ntp_dns_found(const char *hostname, const ip_addr_t *ipaddr, void *arg)
{
    ...
    if (ipaddr)
    {
        ...
        PICO_LOGI(TAG, "NTP DNS: '%s' resolvido para %s", hostname, ipaddr_ntoa(ipaddr));
        ntp_request(state);
    }
    else
    {
        PICO_LOGE(TAG, "NTP DNS: Falha ao resolver '%s'", hostname);
...
static void ntp_recv(void *arg, struct udp_pcb *pcb, struct pbuf *p, const ip_addr_t *addr, u16_t port)
{
    ...
    {
        PICO_LOGE(TAG, "NTP resposta inválida recebida.");
        ntp_result(state, -1, NULL);
    }
    pbuf_free(p);
}
...
static void request_worker_fn(__unused async_context_t *context, async_at_time_worker_t *worker)
{
    ...
    {
        PICO_LOGI(TAG, "NTP DNS: Falha ao solicitar DNS para '%s'", ntp_url);
        ntp_result(state, -1, NULL);
    }
}
...
ntp_t *ntp_init(const char ntp_url_[], const int UTC_offset_seconds)
{
    ...
    if (!state)
    {
        PICO_LOGE(TAG, "Falha ao alocar memória para o cliente NTP.");
        return NULL;
    }
    state->ntp_pcb = udp_new_ip_type(IPADDR_TYPE_ANY);
    if (!state->ntp_pcb)
    {
        PICO_LOGE(TAG, "Falha ao criar PCB UDP para o cliente NTP.");
        free(state);
        return NULL;
    }
